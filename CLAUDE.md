...
All source under `compiler-core/compiler/`:
...
The `Value` struct (`compiler-core/compiler/value.h`) uses a `type` enum:
```
VALUE_INT, VALUE_FLOAT, VALUE_STRING, VALUE_BOOL,
VALUE_NONE, VALUE_ARRAY, VALUE_STRUCT, VALUE_POINTER
```

Scope is managed via `compiler-core/compiler/scope.c` (`Scope` struct with `scope_define()`, `scope_lookup()`, scope chaining).

## Adding Features

### New token
1. `compiler-core/compiler/lexer/token.h` - add to `TokenType` enum
2. `compiler-core/compiler/lexer/lexer.c` - add to `keyword_names[]` + `keyword_types[]` (keywords), or handle in main switch (symbols)
3. `compiler-core/compiler/i18n/i18n.c` - add Mongolian translation

### New AST node
1. `compiler-core/compiler/ast/ast.h` - add to `ASTNodeType`, add struct to the `union`, declare factory function
2. `compiler-core/compiler/ast/ast.c` - implement factory + `ast_free` case + `ast_print` case
3. `compiler-core/compiler/parser/parser.c` - build the node
4. `compiler-core/compiler/interpreter/interpreter.c` - evaluate/execute the node
5. `compiler-core/compiler/transpile/transpile.c` - emit C code for AOT mode (optional)
...
- Keywords are embedded in lexer's keyword_names[] array as pairs (EN at even indices, MN at odd) in `compiler-core/compiler/lexer/lexer.c`
...
- Module imports (`import std.math`) are converted to file paths (`stdlib/math.hn`) in `compiler-core/cli/main.c`.
